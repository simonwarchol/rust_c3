...
// Import the natural logarithm of 2 as a constant
...
impl C3 {
    pub fn new() -> C3 {
        ...
        let color_vec: Vec<i64>  = c3_color::get_c3_color();
        let num_rows = color_vec.len() / 3;
        let num_cols = 3;
        let mut _color = Array2::zeros((num_rows, num_cols));
        for (i, val) in color_vec.iter().enumerate() {
            _color[[i / num_cols, i % num_cols]] = *val as f64;
        }
        let _c = _color.shape()[0];


        let a_vec: Vec<f64> = c3_a::get_c3_a();
        let _a: Array1<f64> = Array1::from(a_vec);


        let mut _t = HashMap::new();

        let t_vec: Vec<i64> = c3_t::get_c3_t();
        ...
        let _terms: Vec<String> = c3_terms::get_c3_terms();
        let _w = _terms.len();
        ...
        let pts = _color
            .outer_iter()
            .enumerate()
            .map(|(i, row)| Item { point: [row[0], row[1], row[2]], id: i })
            .collect();
    ...
    fn analyze_palette(&self, palette: Array2<f64>) -> Vec<HashMap<&str, f64>> {
        palette
            .outer_iter()
